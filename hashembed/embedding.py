...
#From https://github.com/YannDubs/Hash-Embeddings/blob/master/hashembed/embedding.py

#I've made some changes to make this pickle-able
...
class HashFamily():
    ...
    def draw_hash(self, a=None, b=None):
        ...
        if self.mask_zero:
            ...
            #Insead of returning a lambda, return parameters and indicator for which function
            #return lambda x: ((a * x + b) % self.moduler) % (self.bins - 1) + 1
            return (0, a, b, self.moduler, self.bins)

            # return lambda x: 0 if x == 0 else ((a*x + b) % self.moduler) % (self.bins-1) + 1
        else:
            #Insead of returning a lambda, return parameters and indicator for which function
            #return lambda x: ((a * x + b) % self.moduler) % self.bins
            return (1, a, b, self.moduler, self.bins)

    def draw_hashes(self, n, **kwargs):
        """Draws n hash function from the family."""
        return [self.draw_hash() for i in range(n)]

    def hash0(x, a, b, moduler, bins):
        return ((a * x + b) % moduler) % (bins - 1) + 1

    def hash1(x, a, b, moduler, bins):
        return ((a * x + b) % moduler) % bins

    def applyHash(h, input):
        if h[0] == 0:
            return HashFamily.hash0(input, *h[1:])
        else:
            return HashFamily.hash1(input, *h[1:])
...
class HashEmbedding(nn.Module):
    ...
    def aggregate(self, x):
        ...
        elif self.aggregation_mode == 'concatenate':
            # little bit quicker than permute/contiguous/view
            return torch.cat([x[:, :, :, i] for i in range(self.num_hashes)], dim=-1)
    ...
    def forward(self, input):
        ...
        idx_shared_embeddings = torch.stack([HashFamily.applyHash(h, input).masked_fill_(input == 0, 0) for h in self.hashes], dim=-1)
